...
## 4. Signal Generation — 3 Mandatory Filters

For each non-excluded, non-tracked, non-cooldown symbol, the scanner runs `_analyse()`. ALL 3 filters must pass for a signal to fire.
...
After ALL 3 main filters pass, the scanner collects extra context data. Each piece is wrapped in its own `try/except` — **failure of any piece does NOT block the signal from firing**. This data is for analysis only.
